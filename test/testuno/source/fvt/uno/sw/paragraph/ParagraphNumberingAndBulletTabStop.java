...
public class ParagraphNumberingAndBulletTabStop {
	 ...
	 * 2.insert some text
	 * 3.set paragraph numbering tab stop position
	 * 4.save and close the document
	 * 5.reload the saved document and check the paragraph numbering tab stop position
	 */	
	@Test
	public void testNumberingBulletTabStop() throws Exception {
		...
		propsRule[0].Name = "ListtabStopPosition";
		propsRule[0].Value = 5001;	
		...
		assertEquals("assert numbering and bullet","ListtabStopPosition",propsRule_assert_odt[8].Name);
		assertEquals("assert numbering and bullet",5001,propsRule_assert_odt[8].Value);
		...
		//verify paragraph numbering and bullet alignment
		assertEquals("assert numbering and bullet","ListtabStopPosition",propsRule_assert_doc[8].Name);
		assertEquals("assert numbering and bullet",5001,propsRule_assert_doc[8].Value);
	}
}
